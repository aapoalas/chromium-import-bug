...
const getRuleForUrl = (stylesheetUrl) => `@import url("${stylesheetUrl}");`;
...
const main = async () => {
  ...
  loadButton.addEventListener("click", async () => {
    const { default: { data, stylesheets } } = await load("/entries/bar.js")
      .catch((err) => {
    ...
    const { default: { data: data2 } } = await load(
      "/entries/myProvidedData.js",
    );
